                ...
                onTap: coupon.discount != 0 ? null : () {
                  if(widget._couponController.text.isNotEmpty && !coupon.isLoading) {
                    if(coupon.discount! < 1) {
                      coupon.applyCoupon(widget._couponController.text, widget.amountWithoutVat).then((discount) {
                      if(context.mounted){
                        context.pop();
                        if (discount! > 0) {
                  ...
                  itemBuilder: (context, index) {
                    return Padding(padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeLarge), child: InkWell(
                      ...
                      child: Container(
                        ...
                        alignment: Alignment.center,
                        decoration: BoxDecoration(
                          color: ColorResources.getSecondaryColor(context).withValues(alpha:0.05),
                          border: Border.all(color: ColorResources.getSecondaryColor(context).withValues(alpha:0.3)),
                          borderRadius: BorderRadius.circular(Dimensions.radiusDefault),
                        ),
